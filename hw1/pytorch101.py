...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    for j in range(start, stop+1):
        if (j % 10 == 0):
            y = 1
    if (y == 1):
        x = torch.arange(start+i, stop+1, 10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = torch.empty([0,])
    idx2 = torch.arange(x.shape[0]-1, -1, -1)
    y = x[idx2, :]
##########################################################################
#                            END OF YOUR CODE                            #
##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    means = torch.sum(x, dim=0)/x.shape[0]
    # calculatestd
    squared_diff = (x - means)**2
    mean_squared_diff = torch.sum(squared_diff, dim=0) / (x.shape[0]-1)
